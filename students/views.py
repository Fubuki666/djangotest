...
def listone(request): 
    try: 
        unit = student.objects.get(stdName="Fubuki") #讀取一筆資料
    except:
        errormessage = " (讀取錯誤!)"
...
def post(request):
    ...
    if request.method == "POST":
        # 接收傳送資料
        mess = request.POST['stdName'] + " , " + request.POST['stdID'] + " , " + request.POST['stdSex'] + " , " + request.POST['stdBirth'] + " , " + request.POST['stdEmail'] + " , " + request.POST['stdPhone'] + " , " + request.POST['stdAddress']
    else:
        mess = "表單資料尚未送出!"
